...
export function AdminDashboard(store) {
    ...
    const studentsCount = (state.adminStudentList || state.students || []).length;
    const pendingSubmissions = (state.submissions || []).filter(s => s.status === 'pending');
    const pendingTasks = pendingSubmissions.length;
    
    return `
        <div style="animation: slideUpFade var(--transition-slow);">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 40px;">
                <div>
                    <h1 style="margin: 0; font-size: 2.25rem;">أهلاً بك، ${state.user?.name || ''} ✨</h1>
                    <p style="margin: 4px 0 0 0; color: var(--text-secondary); font-size: 1.05rem;">الله يجزاك خير على مبذولاتك !! 🔥</p>
                </div>
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 32px;">
                <div class="card" style="padding: 0; overflow: hidden;">
                    ...
                    ${pendingTasks > 0 ? pendingSubmissions.slice(0, 3).map(sub => `
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 20px 32px; border-bottom: 1px solid var(--border-color); transition: background var(--transition-fast);" onmouseover="this.style.background='var(--bg-surface-hover)'" onmouseout="this.style.background='transparent'">
                            <div>
                                <div style="font-weight: 600; font-size: 1.1rem;">${sub.taskTitle}</div>
                                <div style="font-size: 0.95rem; color: var(--text-secondary); margin-top: 4px;">الطالب: ${sub.studentName}</div>
                            </div>
                            <a href="#/admin/tasks" class="btn btn-secondary" style="padding: 8px 16px; font-size: 0.95rem; font-weight: 600;">تقييم المهمة</a>
                        </div>
                    `).join('') : `
                        <p style="color: var(--text-tertiary); text-align: center; padding: 48px 0; font-size: 1.05rem;">لا توجد تسليمات جديدة بانتظار التقييم.</p>
                    `}
                </div>
